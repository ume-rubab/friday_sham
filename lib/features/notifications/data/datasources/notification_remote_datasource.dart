...
class NotificationRemoteDataSourceImpl implements NotificationRemoteDataSource {
  ...
  Future<void> saveNotification(NotificationModel notification) async {
    try {
      ...
      print('✅ Notification saved for child: ${notification.childId}');
    } catch (e) {
  ...
  Stream<List<NotificationModel>> _streamAllChildrenNotifications(String parentId) {
    ...
    Future<List<NotificationModel>> fetchAllNotifications() async {
      try {
        ...
        print('📡 [NotificationStream] Found ${childrenSnapshot.docs.length} children');

        if (childrenSnapshot.docs.isEmpty) {
          print('📡 [NotificationStream] No children found, returning empty list');
        ...
        for (final childDoc in childrenSnapshot.docs) {
          try {
            print('📡 [NotificationStream] Checking notifications for child: ${childDoc.id}');
            ...
            print('📡 [NotificationStream] Found ${notificationsSnapshot.docs.length} notifications for child ${childDoc.id}');
            ...
            print('❌ Error getting notifications for child ${childDoc.id}: $e');
          }
        }

        // Sort by timestamp descending
        allNotifications.sort((a, b) => b.timestamp.compareTo(a.timestamp));
        print('📡 [NotificationStream] Total notifications: ${allNotifications.length}');
        return allNotifications;
      } catch (e) {
        print('❌ Error in _streamAllChildrenNotifications: $e');
